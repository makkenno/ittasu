...
export function SelectionOverlay({
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      // Auto-panning logic
      const { panX, panY } = calculateAutoPanVelocity(e.clientX, e.clientY);
  ...
  const handlePointerUp = useCallback(
    (e: React.PointerEvent) => {
      ...
      // クリック判定 (移動距離が小さい場合)
      if (dist < 5) {
        handleSelectionClick(e, rfInstance, nodes, onNodeClick, onPaneClick);
      } else {
        handleSelectionDrag(
          e,
          selectionBox,
...
function calculateAutoPanVelocity(clientX: number, clientY: number) {
...
function handleSelectionClick(
  e: React.PointerEvent,
  rfInstance: ReactFlowInstance,
  nodes: SelectionOverlayProps["nodes"],
