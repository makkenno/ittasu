...
import { Slugger } from "../../../lib/markdown-utils";
...
export function MarkdownPreview({ value, className }: MarkdownPreviewProps) {
  ...
  // Reset slugger on every render to ensure consistent ID generation from the top
  ...
  const components: Components = useMemo(
    () => ({
      h1: ({ children }) => {
        ...
        const id = slugger.current.slug(text);
        return (
          <h1
            id={id}
            className="text-2xl font-bold mb-4 text-foreground scroll-mt-20"
          >
            {children}
          </h1>
        );
      ...
      h2: ({ children }) => {
        ...
        return (
          <h2
            id={id}
            className="text-xl font-bold mt-8 mb-4 border-b border-border pb-2 text-foreground scroll-mt-20"
          >
            {children}
          </h2>
        );
      ...
      h3: ({ children }) => {
        ...
        return (
          <h3
            id={id}
            className="text-lg font-bold mt-6 mb-3 text-foreground scroll-mt-20"
          >
            {children}
          </h3>
        );
      ...
      h4: ({ children }) => {
        ...
        return (
          <h4
            id={id}
            className="text-base font-bold mt-6 mb-3 text-foreground scroll-mt-20"
          >
            {children}
          </h4>
        );
      ...
      h5: ({ children }) => {
        ...
        return (
          <h5
            id={id}
            className="text-sm font-bold mt-4 mb-2 text-foreground scroll-mt-20"
          >
            {children}
          </h5>
        );
      ...
      h6: ({ children }) => {
        ...
        return (
          <h6
            id={id}
            className="text-sm font-bold mt-4 mb-2 text-foreground scroll-mt-20 text-muted-foreground"
          >
            {children}
          </h6>
        );
  ...
  return (
    <div
      className={cn(
        "w-full h-full p-6 overflow-y-auto bg-background text-foreground",
        className,
      )}
    >
      <ReactMarkdown
